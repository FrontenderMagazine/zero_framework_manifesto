...
фреймворком. На самом деле у кода есть градиент, который начинается с простого
фрагмента, например Gist, и потом движется ко всё более крупной коллекции кода,
достигая библиотеки, и, наконец, фреймворка:
...
они абстрагируються от платформы, так что вы
...
[абстракции протекают][6]. Таким образом, вы должны знать HTML+CSS+JS, потому что
в какой-то момент ваша программа не будет работать так, как вы ожидаете, и вы
будете копать вниз через все слои во фреймворке, чтобы понять что не так,
...
Я склонен сичтать, что изучение фреймворка — это как картографирование айсберга,
для того, чтобы
использовать его, вам нужно узнать всё о нем, приложить усилия и изучить все
его особености, и в конечном счете всё будет напрасно, потому что
...
HTML5. Но нам неоходимо не только это, есть ещё и
...
[короткое полифилл для каждой функции][15].
...
при работе, и выдал бы мне только из всего полифилла HTML5 только то, что нужно
дл моего проекта.
...
включают обе эти библиотеки полифиллов?
...
проект, поэтому наша работа с этими вещами должна изменится. Конечно, продолжайте
хранить код в Git, но нужны ли вам накладные расходы в виде GitHub? Что-то
более легковесное, ближе к Gist, чем к проекту, подошло бы лучше. А как
минимизировать/[вулканизовать][19] весь этот кода в подходящий для
...
1. Идиомы и принципы построения повтоно используемых компонентов.
...
знать последнюю модель новейшого фреймворка, вместо этого мы просто будет работать
напрямую с платформой, загружая элементы и библиотеки для своих нужд, и тратя
своё время на написание приложений, а не на картографирование айсбергов.
...
**О:** Нет, это не то, что я сказал, я был очень осторожен, чтобы разграничить
библиотеки и фреймворки. Библиотека, обеспечивающая ортогональный
...
не нужно принимать весь фреймворк, чтобы получить привязку данных, для этого есть
